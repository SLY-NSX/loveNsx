...
function renderCuriosityList(status) {
    ...
    list.innerHTML = data.slice().reverse().map(letter => {
        ...
        if (isDraft) {
            ...
            // 检查是否有_enteredYes标记（如果有且为true，说明进入了YES阶段）
            const enteredYes = letter._enteredYes || false;
            if (enteredYes) {
                statusText = `✅ 已填写 · ${answered}/${total} 已答`;
            } else {
                statusText = `❌ 投递失败 · ${answered}/${total} 已答`;
            }
        } else {
            statusText = '⏳ 等待回复中';
        }
...
window.openCuriosityDetail = function(questionnaire, editable, numPart) {
    ...
    // 确保 questions 中的每个问题都有 _status、_selectedOptions、_askedBack
    const questionsWithStatus = (questionnaire.questions || []).map(q => ({
        text: q.text || '',
        type: q.type || 'single',
        options: q.options || [],
        _status: q._status || 'pending',
...
function buildResult(originalQuestionnaire, questions, status, enteredYes) {
    ...
    // 构建结果对象（包含完整的 questions 数据，含 _status、_selectedOptions、_askedBack）
    const result = {
...
window.handleDelivery = async function() {
    ...
    // 3. 创建 A-1 版本问卷（原始 questions 不含 _status 字段）
    ...
    (async function() {
        try {
            ...
            // ✅ 关键：result 中的 questions 已包含 _status 等字段
...
async function handleDeliveryResult(result, originalQuestionnaire) {
    ...
    // 2. 存入新版本（result 包含完整的 _status 数据）
